...
% Import Parasolid CAD data.
classdef Parasolid < handle
    %% CST Interface specific functions.
    methods(Access = ?CST.Project)
        % Only CST.Project can create a CST.Parasolid object.
        function obj = Parasolid(project, hProject)
            obj.project = project;
            obj.hParasolid = hProject.invoke('Parasolid');
            obj.history = [];
        end
    end
    methods
    ...
    methods
        ...
        function Read(obj)
            ...
            % Prepend With Parasolid and append End With
            obj.history = [ 'With Parasolid', newline, ...
                                obj.history, ...
                            'End With'];
            obj.project.AddToHistory(['import Parasolid: ', obj.filename], obj.history);
            obj.history = [];
        end
    end
...
% parasolid = project.Parasolid();
%     parasolid.Reset
%     parasolid.FileName('.\example.x_t');
%     parasolid.ScaleToUnit(1)
%     parasolid.Id(1)
%     parasolid.Healing(1)
%     parasolid.ImportAttributes(1)
%     parasolid.Curves(1)
%     parasolid.ImportCurveAttributes(1)
%     parasolid.Read
