...
classdef ResultTree < handle
    ...
    methods
        ...
        function resultObject = GetResultFromTreeItem(obj, treeItemName, resultID)
            % Returns a result object containing the data specified by 'treeItemName' and 'resultID'
            % . The return value can be a Result0D, a Result 1D or a Result 1D Complex object.  In
            % case no data exists, a reference to an object is returned that is nothing. This can be
            % queried via the VBA keyword Nothing (e.g. If(myObject Is Nothing)Then...). The method
            % returns an error, if the tree item does not exist or the Result ID is invalid.
        ...
        function resultObject = GetImpedanceResultFromTreeItem(obj, treeItemName, resultID)
            ...
            hResult = obj.hResultTree.invoke('GetImpedanceResultFromTreeItem', treeItemName, resultID);
            type = hResult.invoke('GetResultObjectType');
            switch(type)
                case {'0D', '0DC'}
                    resultObject = CST.Result0D(obj.project, hResult);
