...
for(i = 1:length(objectnames))
    ...
    if(strcmpi(objectname, 'Result0D') || strcmpi(objectname, 'Result1D') || strcmpi(objectname, 'Result1DComplex') || strcmpi(objectname, 'Result3D'))
        ...
        methodstring = [methodstring, sprintf('        function %s = %s(obj, resultname)\n', lobjectname, funcname), ...
                                      ...
                                      sprintf('%%                 obj.%s = CST.%s(obj, obj.hProject, resultname);\n', lobjectname, objectname), ...
                                      sprintf('%%             end\n'), ...
                                      sprintf('%%             %s = obj.%s;\n', lobjectname, lobjectname), ...
                                      sprintf('            %% Each %s can be different depending on resultname, so don''t store it.\n', lobjectname), ...
